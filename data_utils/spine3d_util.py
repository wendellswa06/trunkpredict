...
DATA_PATH = os.path.join(ROOT_DIR, 'Spine')
...
def collect_point_label(spine_pre, spine_post, out_filename, file_format='txt'):
    ...
    """
    points_list = []
    for f in glob.glob(os.path.join(spine_pre, '*.ply')):
        # fileID = f"{os.path.basename(f).split('_')[0]}_{os.path.basename(f).split('_')[1]}"
        fileID = os.path.basename(f).split('.')[0]
        postfilelist = os.listdir(spine_post)
        # print(f)
        if f"{fileID}.post_normilized.ply" not in postfilelist: # note: in some room there is 'staris' class..
            continue

        prepoints = np.loadtxt(f, skiprows = 7)
        postpoints = np.loadtxt(os.path.join(spine_post, f"{fileID}.post_normilized.ply"), skiprows = 7)
        points = np.concatenate([prepoints, postpoints], 1)
        
        points_list.append(np.concatenate([prepoints, postpoints], 1)) # Nx7
        
    # print(points)
    print(points_list)
    # print(len(points_list[0][0]))
    ...
    if file_format=='txt':
        ...
        np.save(out_filename, data_label)#this is the execution code
    else:
