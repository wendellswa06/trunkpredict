...
DATA_PATH = os.path.join(ROOT_DIR, 'FBST')
...
def collect_point_label(trunk_pre, trunk_post, out_filename, file_format='txt'):
    ...
    """
    points_list = []
    for f in glob.glob(os.path.join(trunk_pre, '*.obj')):
        fileID = f"{os.path.basename(f).split('_')[0]}_{os.path.basename(f).split('_')[1]}"
        postfilelist = os.listdir(trunk_post)
        # print(f)
        if f"{fileID}_Postop_Analyse_rgb_gt.obj" not in postfilelist: # note: in some room there is 'staris' class..
            continue

        prepoints = np.loadtxt(f)
        postpoints = np.loadtxt(os.path.join(trunk_post, f"{fileID}_Postop_Analyse_rgb_gt.obj"))
